...
What's new:
- Online Sound Library browser for GitHub folder (MP3s)
...
GITHUB_RAW_PREFIX = "https://raw.githubusercontent.com/OptionallyBlueStudios/KeyCL/main/sounds"
...
class KeyCLApp:
    ...
    def browse_sound_library(self):
        """Browse and download sounds from the GitHub 'sounds' folder (MP3 files)."""
        if requests is None:
        ...
        # Only MP3s in that folder per your note
        mp3s = [f for f in files if f.get("name", "").lower().endswith(".mp3")]
        if not mp3s:
            messagebox.showinfo("No Sounds", "No MP3 files were found in the online library.")
            return

        # Build a browser window
        win = ctk.CTkToplevel(self.root)
        win.title("Browse Online Sound Library")
        win.geometry("700x520")

        # Search box
        search_var = ctk.StringVar(value="")
        search_entry = ctk.CTkEntry(win, textvariable=search_var, placeholder_text="Search sounds...")
        ...
        def render_list():
            ...
            for f in mp3s:
                ...
                ctk.CTkLabel(row, text=title, font=ctk.CTkFont(size=14, weight="bold")).pack(
                    side="left", padx=10, pady=8
                )

                def do_download(url=f.get("download_url"), title=title):
                    # Construct a minimal keyclsound for this MP3
                    meta = {
                        "title": title,
                        "author": "KeyCL Library",
                        "description": f"{title} from online library.",
                        "tags": ["library"],
                        "url": url
                    }
                    self.install_keyclsound_from_metadata(meta)

                ctk.CTkButton(row, text="Download", width=100, command=do_download).pack(
                    side="right", padx=10
                )
    ...
    def install_keyclsound_from_metadata(self, meta: dict):
        """Given parsed metadata, download the MP3 and save a .keyclsound alongside it."""
        ...
        try:
            ...
            if ext not in [".mp3", ".wav", ".ogg", ".m4a"]:
                # Fallback to .mp3 since your library is MP3
                ext = ".mp3"
...
• Browse & install sounds from online library
• Install .keyclsound packages
