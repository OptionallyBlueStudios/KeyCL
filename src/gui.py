    ...
    def setup_tray(self):
        ...
        menu_items = [
            ...
            item("Quick Volume", pystray.Menu(
                item("Volume 25%", lambda: self.set_volume(0.25)),
                item("Volume 50%", lambda: self.set_volume(0.50)),
                item("Volume 75%", lambda: self.set_volume(0.75)),
                item("Volume 100%", lambda: self.set_volume(1.0))
            ))
        ]
        ...
        if sound_list:
            menu_items.append(item("Sound Library", pystray.Menu(
                *[item(sound_name, lambda _, name=sound_name: self.set_sound(name))
                for sound_name in sound_list[:10]]
            )))
